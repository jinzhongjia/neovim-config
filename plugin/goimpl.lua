...
function M.goimpl()
    ...
    if vim.fn.executable("impl") ~= 1 then
        notify(
            "impl command not found. Install it with: go install github.com/josharian/impl@latest",
    ...
    if not has_workspace_symbol_support(bufnr) then
        notify(
            "No LSP client with workspace symbol support attached. Make sure gopls is running.",
            vim.log.levels.ERROR
        )
        return
    end
