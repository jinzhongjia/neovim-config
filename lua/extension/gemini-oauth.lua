...
-- OAuth flow constant configuration (from opencode-gemini-auth)
local OAUTH_CONFIG = {
...
local function generate_random_string(length)
    ...
    if not bytes then
        log:error("Gemini OAuth: Unable to generate secure random bytes")
        return nil
    end
...
local function sha256_base64url(input)
    ...
    if not hash_binary then
        log:error("Gemini OAuth: Unable to generate PKCE hash")
        return nil
    end
...
local function generate_pkce()
    ...
    log:debug("Gemini OAuth: Generated verifier length: %d", #verifier)
    ...
    log:debug("Gemini OAuth: Generated challenge length: %d", #challenge)
...
local function get_token_file_path()
    ...
    if not data_path then
        log:error("Gemini OAuth: Unable to determine data directory")
        return nil
    end

    local path_sep = package.config:sub(1, 1)
    return data_path .. path_sep .. "gemini_oauth.json"
end
...
local function load_tokens()
    ...
    if not success or not content or #content == 0 then
        log:debug("Gemini OAuth: Unable to read token file or file is empty")
        return false
    end
    ...
    if decode_success and data then
        ...
        log:warn("Gemini OAuth: Invalid token file format")
        return false
    end
end
...
local function save_tokens(access_token, refresh_token, expires, project_id)
    ...
    if not success then
        log:error("Gemini OAuth: Unable to encode token data")
        return false
    end
    ...
    if success then
        ...
        log:error("Gemini OAuth: Failed to save tokens: %s", err or "unknown error")
        return false
    end
end
...
local function refresh_access_token()
    ...
    if response.status >= 400 then
        log:error("Gemini OAuth: Token refresh failed, status %d: %s", response.status, response.body or "no body")
        ...
        if decode_ok and error_data and error_data.error == "invalid_grant" then
            log:warn("Gemini OAuth: Refresh token revoked. Please run :GeminiOAuthSetup to reauthenticate")
            _access_token = nil
            _refresh_token = nil
            _token_expires = nil
            _project_id = nil
...
local function exchange_code_for_tokens(code, verifier)
    ...
    log:debug("Gemini OAuth: Code length: %d, Verifier length: %d", #code, #verifier)
    ...
    if response.status >= 400 then
        log:error("Gemini OAuth: Token exchange failed, status %d: %s", response.status, response.body or "no body")
        return false
    end
    ...
    if not token_data.access_token then
        log:error("Gemini OAuth: Missing access_token in response")
        return false
    end

    if not token_data.refresh_token then
        log:error("Gemini OAuth: Missing refresh_token in response")
        return false
    end
...
local function generate_auth_url()
    ...
    log:debug("Gemini OAuth: PKCE verifier: %s", pkce.verifier:sub(1, 20) .. "...")
    log:debug("Gemini OAuth: PKCE challenge: %s", pkce.challenge)
...
local function get_access_token()
    ...
    log:error("Gemini OAuth: Access token not available. Please run :GeminiOAuthSetup to authenticate")
    return nil, nil
end
...
local function start_oauth_server(verifier, callback)
    ...
    if not server then
        log:error("Gemini OAuth: Failed to create TCP server")
    ...
    if not bind_ok then
        log:error("Gemini OAuth: Failed to bind to port %d: %s", OAUTH_CONFIG.CALLBACK_PORT, bind_err or "unknown")
        server:close()
    ...
    local listen_ok, listen_err = server:listen(128, function(err)
        if err then
            log:error("Gemini OAuth: Server listen error: %s", err)
            return
        end
        ...
        client:read_start(function(read_err, chunk)
            if read_err then
                log:error("Gemini OAuth: Read error: %s", read_err)
                client:close()
                return
            end
            ...
            if chunk then
                ...
                if string.find(request_data, "\r\n\r\n") then
                    ...
                    if path and string.find(path, "/oauth2callback") then
    ...
    if not listen_ok then
        log:error("Gemini OAuth: Failed to listen: %s", listen_err or "unknown")
        server:close()
        callback(nil, "Failed to start server")
        return
    end

    log:debug("Gemini OAuth: Server listening on port %d", OAUTH_CONFIG.CALLBACK_PORT)
...
local function setup_oauth()
    ...
    if not auth_data then
        vim.notify("Unable to generate Gemini OAuth authorization URL, please check logs.", vim.log.levels.ERROR)
        return false
    end
...
vim.api.nvim_create_user_command("GeminiOAuthSetup", function()
    setup_oauth()
end, {
    desc = "Setup Gemini OAuth authentication",
})

vim.api.nvim_create_user_command("GeminiOAuthStatus", function()
    load_tokens()
    if not _refresh_token then
        vim.notify("Gemini OAuth: Not authenticated. Run :GeminiOAuthSetup to authenticate.", vim.log.levels.WARN)
        return
    end
    ...
    desc = "Check Gemini OAuth status",
})

vim.api.nvim_create_user_command("GeminiOAuthClear", function()
    local token_file = get_token_file_path()
    ...
    desc = "Clear stored Gemini OAuth tokens",
})

-- Create adapter using Code Assist API
local adapter = {
    name = "gemini_oauth",
    ...
    handlers = {
        ...
        set_body = function(self, payload)
            ...
            -- Return the full Code Assist API format
            return {
