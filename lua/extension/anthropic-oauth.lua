...
---Finds the data path for storing OAuth tokens
---@return string|nil
...
local function get_token_file_path()
    ...
    if not data_path then
        log:error("Anthropic OAuth: Could not determine data directory")
        return nil
    end

    return data_path .. "/anthropic_oauth.json"
end
...
local function create_api_key(access_token)
    ...
    if response.status >= 400 then
        log:error(
            "Anthropic OAuth: Create API key failed with status %d: %s",
            response.status,
...
local function exchange_code_for_api_key(code, verifier)
    ...
    if response.status >= 400 then
        log:error(
            "Anthropic OAuth: Token exchange failed with status %d: %s",
            response.status,
            response.body or "no body"
        )
        return nil
...
local function setup_oauth()
    ...
    vim.ui.input({
        ...
        if api_key then
            ...
            vim.notify(
                "Anthropic OAuth authentication failed. Please check the logs and try again.",
